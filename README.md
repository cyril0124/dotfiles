...
- Install Herdr plugins listed in `config/herdr/plugins.txt` when `herdr` is available (GitHub `owner/repo` via install; `link:path` for local plugins)
...
│   ├── herdr/       # Herdr config, plugins.txt, local plugins (file-linked; runtime data stays in ~/.config/herdr)
